...
void 
LteV2xHelper::DeactivateSidelinkBearer (Time deactivationTime, NetDeviceContainer ues, Ptr<LteSlTft> tft)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_lteHelper, "Sidelink deactivation requires LteHelper to be registered with the LteV2xHelper");
  Simulator::Schedule (deactivationTime, &LteV2xHelper::DoDeactivateSidelinkBearer, this, ues, tft);
}

void 
LteV2xHelper::DoDeactivateSidelinkBearer (NetDeviceContainer ues, Ptr<LteSlTft> tft)
{
  NS_LOG_FUNCTION (this);
  m_lteHelper->DeactivateSidelinkBearer (ues, tft);
}
